    ...
    public class AdminController : ControllerBase
    {
        ...
        public Category UploadImage()
        {
            try
            {
                ...
                var fullPath = Path.Combine("C:\\Users\\Dash Arabia\\Desktop\\Hyper_Market\\src\\assets\\images", ImageName);
        ...
        public About UploadImageAbout()
        {
            try
            {
                var Image = Request.Form.Files[0];
                var ImageName = Guid.NewGuid().ToString() + Image.FileName;
                var fullPath = Path.Combine("C:\\Users\\Dash Arabia\\Desktop\\Hyper_Market\\src\\assets\\mages", ImageName);
                using (var stream = new FileStream(fullPath, FileMode.Create))
                {
                    Image.CopyTo(stream);
                }
